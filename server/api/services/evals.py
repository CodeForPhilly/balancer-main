...
if __name__ == "__main__":
    ...
    if not all(
        model in ModelFactory.HANDLERS.keys() for model in df_config["Model"].unique()
    ):
        raise ValueError(
            f"Unsupported model(s) found in config: {set(df_config['Model'].unique()) - set(ModelFactory.HANDLERS.keys())}"
        )
    ...
    for index, row in df_in.iterrows():
        ...
        df_evals = pd.concat(
            [
                df_evals,
                evaluate_response(
                    row["Model"], row["Query"], row["Context"], row["Reference"]
                ),
            ],
            axis=0,
        )
