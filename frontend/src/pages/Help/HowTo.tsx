  ...
  return (
    <Layout>
      <div className="mt-20 flex w-full max-w-6xl flex-col items-center justify-center px-8">
        <div className="mt-10 flex flex-col rounded-xl border-2 border-blue-100 bg-neutral-50 px-4 py-4 shadow-md">
          <h3 className="flex items-center justify-center py-2 text-xl">
            {howToData.title}
          </h3>
          ...
          <div className="flex px-6">
            <ol>
              {howToData.instructions.map((text, index) => {
                return <li className="font-satoshi text-sm text-gray-700 py-1"
                key={index}>{text}</li>;
              })}
            </ol>
          </div>
